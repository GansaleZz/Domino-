...
Interface
	...
	public
		Constructor Init();
		Procedure MakeTable(bool:boolean);
		...
		Procedure TakeFromMarket(bool:boolean);
		Function ChooseDice(boolCH:boolean):Ptr;
		Procedure TakeDice(boolCH:boolean);
		Procedure PartOfDesk();
		Procedure RotationOfDice(x,y:integer;var Horizontal:boolean; var count:word);
		Procedure PutDice(x,y:integer;var DeskPtr:ptr;boolCH:boolean);
		Procedure Table();
	private
...
//--------------------------------------------------Realization of GraphicPart---------------------------------------------------------------------------------
    ...
    Procedure GraphWin.MakeTable(bool:boolean);
    var temp:ptr;
        DeskP:DeskPart;
	begin
		setfillstyle(1,Viridian);
	...
	Function GraphWin.ChooseDice(boolCH:boolean):Ptr;
	var first,Current:ptr;
		bool:boolean;
		x,y:integer;
	begin
	...
	Procedure GraphWin.TakeDice(boolCH:boolean);
	var state:word;
		x,y:integer;
		Current,first:ptr;
		f,bool:boolean;
	...
	Procedure GraphWin.PutDice(x,y:integer; var DeskPtr:ptr;boolCH:boolean) ;
	...
	begin
		...
		g.FirstMove(gg,tempDesk);
		if gg then begin
			OutTextXY(50,20,'First move by First player');
			temp:=g.getHand2;
		end else begin
			OutTextXY(50,20,'First move by Second player');
			temp:=g.getHand1;
		end;
		gg:=not gg;
		SetTextStyle(0,0,2);
		setfillstyle(1,Viridian);
		Bar(HandX[1],HandY,HandX[2],getMaxY);
		Hands(temp,250,getMaxY-85);
		...
		repeat
			...
			x:=GetMouseX;
			y:=GetMouseY;	
			if ((g.getHand1 = nil) and gg) or ((g.getHand2 = nil) and not gg) then begin
				writeln('The end!');
				repeat
					x:=GetMouseX;
					y:=GetMouseY;
			...
			if ((EscX[2]>=x) and (EscY[2]<=y)) then WorkWithButtonEsc() else
				if ((x<=getMaxX) and (x>=getMaxX-EscX[2]) and (EscY[2]<=y)) then begin
					TakeFromMarket(gg);
					repeat  
						x:=GetMouseX;
						y:=GetMouseY;
					until((x<getMaxX-EscX[2]) or (EscY[2]>y));
				end else begin
	...
	Procedure GraphWin.TakeFromMarket(bool:boolean);
	var temp:ptr;
		flag,col:boolean;
        x,y:integer;
	begin
		...
		repeat
			x:=GetMouseX;
			y:=GetMouseY;
			if (((x<=getMaxX) and (x>=getMaxX-EscX[2]) and (EscY[2]<=y)) and ( col = false)) then begin
				col:= true;
				setfillstyle(1,DarkGray);
				Bar(getmaxX-EscX[2],EscY[2],getmaxX,EscY[1]); // key Market
				SetColor(black);
			...
			if ((col) and ((getMaxX-EscX[2]>x) or (EscY[2]>y)))then begin
					setfillstyle(1,Teal);
					Bar(getmaxX-EscX[2],EscY[2],getmaxX,EscY[1]); // key Market
					SetColor(black);
					Rectangle(getmaxX-EscX[2],EscY[2],getmaxX,EscY[1]);
			...
			If (((GetMouseButtons and MouseLeftButton = MouseLeftButton)or (GetMouseButtons and MouseRightButton = MouseRightButton)) and ((x>=getMaxX-EscX[2]) and (EscY[2]<=y)) and (g.getMarket<>nil))   Then
			begin
				flag:=true;
				g.TakeFromMarket(bool);
				if bool then
					temp:=g.getHand1
				else
					temp:=g.getHand2;
